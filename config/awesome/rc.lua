...
-- need install awesome-extra first
...
query_dict_cmd = terminal .. " -g 40x25 -e sdcv"
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "b", function() awful.util.spawn("/opt/waterfox/waterfox") end),

    -- my cli search
    awful.key({ modkey }, "s", function() awful.util.spawn(search_cmd) end),
    ...
    -- shutdown
    awful.key({ modkey, "Control" }, "End", function() awful.util.spawn(shutdown_cmd) end),
...
-- load luafilesystem, need install lua-filesystem in debian
require("lfs") 
-- {{{ Run programm once
...
-- Usage Example
--run_once("firefox")
run_once("/opt/waterfox/waterfox")
run_once("emacs")
run_once("workrave")
